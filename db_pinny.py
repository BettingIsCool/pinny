...
def get_rowcount(table: str, date_from: datetime, date_to: datetime, league_ids: list, markets: list, periods: list):
    ...
    st.write(f"SELECT COUNT(event_id) FROM {table} WHERE starts >= '{date_from.strftime('%Y-%m-%d %H:%M:%S')}' AND starts <= '{date_to.strftime('%Y-%m-%d %H:%M:%S')}' AND league_id IN {league_ids} AND market IN {markets} AND period IN {periods}")
